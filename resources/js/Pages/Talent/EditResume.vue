...
const pageTitle = ref("Edit Your Resume");
...
const parsedWhatIDo = ref<
    Array<{ skillset: string; details: string; icon: string | null }>
>([]);
const parsedSkills = ref<Array<{ name: string }>>([]);
const parsedEducations = ref<
    Array<{ institution: string; degree: string; year: string }>
>([]);
const parsedExperiences = ref<
    Array<{ company: string; role: string; duration: string }>
>([]);
...
onMounted(() => {
    ...
    if (props.resume.social_links) {
        parsedSocialLinks.value = JSON.parse(props.resume.social_links);
        form.socialLinks = parsedSocialLinks.value;
    }
    if (props.resume.what_i_do) {
        parsedWhatIDo.value = JSON.parse(props.resume.what_i_do);
        form.whatIDo = parsedWhatIDo.value;
    }
    if (props.resume.skills) {
        parsedSkills.value = JSON.parse(props.resume.skills);
        form.skills = parsedSkills.value;        
    }

    if (props.resume.educations) {
        // Attempt to parse the JSON string
        parsedEducations.value = JSON.parse(props.resume.educations);
        form.educations = parsedEducations.value;
    }
    if (props.resume.experiences) {
        parsedExperiences.value = JSON.parse(props.resume.experiences);
        form.experiences = parsedExperiences.value;
    }
    if (props.resume.projects) {
        parsedProjects.value = JSON.parse(props.resume.projects);
        form.projects = parsedProjects.value;
    }
});
...
const form = useForm({
    ...
    citizen: props.resume.citizen,
    address: props.resume.address,
    favorate_quote: props.resume.favorate_quote,
            ...
            <div class="mx-auto max-w-screen-2xl">
                ...
                <div class="grid grid-cols-1 gap-9 sm:grid-cols-2">
                    <div class="flex flex-col gap-9">
                        <!-- Input Fields Start -->
                        <DefaultCard cardTitle="Edit Resume">
